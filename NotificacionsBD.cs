...
namespace Programa.Dades
{
    class NotificacionsBD
    {
        ...
        public List<Notificacio> TotesLesNoti()
        {
            ...
            if (connection != null)
            {
                ...
                while (reader.Read())
                {
                    ...
                    // Debugging line
                    Console.WriteLine($"Loaded: {notificacio.idNotificacio}, {notificacio.usuari}, {notificacio.matricula}, {notificacio.llegida}, {notificacio.descripcio}");
                }
                reader.Close();
                connection.Close();
            }
        ...
        public void InsertNotiBDD(int idNotificacio, int llegida, string usuari, string matricula, string descripcio)
        {
            MySqlConnection connection = connexio.ConnexioBDD();
            if (connection != null)
            {
